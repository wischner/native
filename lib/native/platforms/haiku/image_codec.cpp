...
namespace native::detail
{
    decoded_image decode_image(
        ...
        std::unique_ptr<BBitmap> source(BTranslationUtils::GetBitmap(&input));
        if (!source || source->InitCheck() != B_OK)
            throw std::runtime_error("Haiku image codec: decode failed");

        const std::size_t width =
            static_cast<std::size_t>(source->Bounds().IntegerWidth()) + 1;
        const std::size_t height =
            static_cast<std::size_t>(source->Bounds().IntegerHeight()) + 1;
        if (width == 0 || height == 0 ||
            ...
            height > static_cast<std::size_t>(
                std::numeric_limits<coord>::max())) {
            throw std::runtime_error(
                "Haiku image codec: dimensions exceed native image limits");
        }
        ...
        const auto *base = static_cast<const std::uint8_t *>(rgba_bitmap.Bits());
        for (std::size_t y = 0; y < height; ++y) {
            const auto *row = base + y * rgba_bitmap.BytesPerRow();
            for (std::size_t x = 0; x < width; ++x) {
                decoded.pixels[y * width + x] = rgba(
                    row[x * 4 + 2],
                    row[x * 4 + 1],
                    row[x * 4],
                    row[x * 4 + 3]);
    ...
    std::vector<std::uint8_t> encode_image(
        ...
        const uint32 output_type = format == image_format::png
            ? B_PNG_FORMAT
            : B_JPEG_FORMAT;
        const status_t status = BTranslatorRoster::Default()->Translate(
            ...
            output_type);
        if (status != B_OK)
            throw std::runtime_error("Haiku image codec: encode failed");

        const auto *bytes = static_cast<const std::uint8_t *>(output.Buffer());
