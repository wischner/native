...
Color values are represented by the `rgba` structure in the public header. It
stores the `r`, `g`, `b`, and `a` channels explicitly as eight-bit values.

`rgba` can be constructed from individual channels or from packed
`0xAABBGGRR` data. Its conversion to `std::uint32_t` uses the same packing
independently of host byte order; it does not rely on a union or native memory
layout.
