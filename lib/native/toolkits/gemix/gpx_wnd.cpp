...
namespace native
{
    gpx_wnd::gpx_wnd(const wnd *window, point offset)
        ...
        _clip = rect(0, 0, window->get_dimensions().w, window->get_dimensions().h);
    }
    ...
    gpx &gpx_wnd::draw_line(point from, point to) {
        ...
        clip_to_vdi(r);
        vsl_color(linux::gemix::runtime.vdi_handle, gem_color(get_ink()));
    ...
    gpx &gpx_wnd::draw_rect(rect r, bool filled) {
        ...
        if (filled) {
            vsf_interior(linux::gemix::runtime.vdi_handle, FIS_SOLID);
            vsf_color(linux::gemix::runtime.vdi_handle, gem_color(get_ink()));
            WORD pxy[4] = {
                static_cast<WORD>(r.p.x),
                static_cast<WORD>(r.p.y),
                static_cast<WORD>(r.p.x + r.d.w - 1),
