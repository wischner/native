...
| Borrowed-page four-edge `tab_view` | Athena directional host (build tested) | Emulated directional host (build tested) | `XmNotebook`, rotated side labels (build tested) | XView/OLGX directional host (build tested) | `WMTabView` top, WINGs-matched directional fallback (tested) | GEM directional host (build tested) | Win32 tab control (build tested) | `BTabView` top/bottom, directional side host (tested) | `NSTabView` + `NSTabViewItem` (build tested) | WIP |
