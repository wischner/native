...
namespace
{
    ...
    native::detail::decoded_image decode_png(
    ...
    native::detail::decoded_image decode_jpeg(
        const std::uint8_t *data,
        std::size_t size) {
        if (size > ULONG_MAX)
            throw std::runtime_error("JPEG decode failed: input is too large");
        ...
        try {
            ...
            validate_dimensions(codec.output_width, codec.output_height);
            if (codec.output_components != 3)
                throw std::runtime_error(
                    "JPEG decode failed: unsupported output color space");

            decoded.width = static_cast<native::dim>(codec.output_width);
            decoded.height = static_cast<native::dim>(codec.output_height);
            decoded.pixels = std::make_unique<native::rgba[]>(
                static_cast<std::size_t>(decoded.width) * decoded.height);
            ...
            while (codec.output_scanline < codec.output_height) {
                ...
                for (std::size_t x = 0; x < decoded.width; ++x) {
                    decoded.pixels[y * decoded.width + x] = native::rgba(
                        row[x * 3],
                        row[x * 3 + 1],
                        row[x * 3 + 2],
                        255);
    ...
    std::vector<std::uint8_t> encode_jpeg(
        ...
        while (codec.next_scanline < codec.image_height) {
            const native::rgba *source =
                pixels + static_cast<std::size_t>(codec.next_scanline) * width;
...
namespace native::detail
{
    decoded_image decode_image(
        ...
        throw std::runtime_error("image codec: input is not PNG or JPEG");
    }
    ...
    std::vector<std::uint8_t> encode_image(
        ...
        throw std::invalid_argument("image codec: unsupported image format");
    }
}
