...
// font_t on macOS: the platform handle (macfont) retains an NSFont and lives
...
namespace
{
    ...
    {
        auto *f = mac::font_bindings.from_a(id);
        if (f)
        {
            [f->ns_font release];
            delete f;
        }
        mac::font_bindings.unregister_by_a(id);
    }

    uint32_t register_font(NSFont *nsfont)
    {
        [nsfont retain];
        auto *h = new mac::macfont();
        h->ns_font = nsfont;
        uint32_t id = next_id();
