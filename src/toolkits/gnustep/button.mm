...
@interface GNUButtonTarget : NSObject
{
@public
...
@implementation GNUButtonTarget
- (void)buttonAction:(id)sender
{
    (void)sender;
    if (_owner)
...
namespace
{
    static NSString *to_nsstring(const std::string &text)
    {
        ...
        if (!value)
            value = [NSString stringWithCString:text.c_str() encoding:NSISOLatin1StringEncoding];
        if (!value)
            value = @"";
        return value;
    }
...
namespace native
{
    ...
    button &button::set_text(const std::string &text)
    {
        ...
        if (_created)
        {
            auto *h = gnustep::button_bindings.from_a(const_cast<button *>(this));
            if (h && h->ns_button)
    ...
    void button::create() const
    {
        ...
        wnd *p = parent();
        if (!p)
            throw std::runtime_error("GNUstep: button requires a parent window.");

        NSWindow *window = gnustep::wnd_bindings.from_b(p);
        if (!window)
            throw std::runtime_error("GNUstep: button parent is not created.");

        NSView *content = [window contentView];
        if (!content)
            throw std::runtime_error("GNUstep: button parent has no content view.");
        ...
        GNUButtonTarget *target = [[GNUButtonTarget alloc] init];
        ...
        auto *h = new gnustep::gnustepbutton();
        h->ns_button = btn;
        h->target = target;
        h->owner = self;
        gnustep::button_bindings.register_pair(self, h);
    ...
    {
        if (!_created)
            throw std::runtime_error("GNUstep: Cannot show button before it is created.");

        auto *h = gnustep::button_bindings.from_a(const_cast<button *>(this));
        if (!h || !h->ns_button)
            throw std::runtime_error("GNUstep: Missing NSButton binding.");
    ...
    void button::destroy() const
    {
        ...
        auto *self = const_cast<button *>(this);
        auto *h = gnustep::button_bindings.from_a(self);
        ...
        if (h)
        {
            ...
            gnustep::button_bindings.unregister_by_a(self);
            delete h;
        }
