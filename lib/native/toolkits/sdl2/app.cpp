...
namespace native
{
    static void render_window_if_needed(native::wnd *wnd) {
        ...
        SDL_Window *sdl_win = linux::sdl2::wnd_bindings.handle_from_object(wnd);
        if (!sdl_win)
            return;

        auto *cache = linux::sdl2::wnd_gpx_bindings.object_from_handle(wnd);
        ...
        if (auto *aw = dynamic_cast<native::app_wnd *>(wnd)) {
            if (aw->menu.id()) {
                auto *sm = linux::sdl2::menu_bindings.object_from_handle(aw->menu.id());
                if (sm)
    ...
    int app::main_loop() {
        ...
        while (running) {
            while (SDL_PollEvent(&event)) {
                ...
                native::wnd *wnd = linux::sdl2::wnd_bindings.object_from_handle(
                ...
                case SDL_MOUSEMOTION:
                {
                    ...
                    linux::sdl2::handle_button_motion(wnd, event.motion.x, event.motion.y);
                ...
                {
                    auto *cache2 = linux::sdl2::wnd_gpx_bindings.object_from_handle(wnd);
                    ...
                    if (event.type == SDL_MOUSEBUTTONDOWN) {
                        if (auto *aw = dynamic_cast<native::app_wnd *>(wnd)) {
                            if (aw->menu.id()) {
                                ...
                                if (sm && linux::sdl2::handle_menu_click(
                                        sm,
                                        event.button.x,
                                        event.button.y,
                                        btn_win_w)) {
                ...
                case SDL_WINDOWEVENT:
                    ...
                    case SDL_WINDOWEVENT_EXPOSED:
                        if (auto *cache = linux::sdl2::wnd_gpx_bindings.object_from_handle(wnd))
                            cache->invalidated = true;
                        break;
