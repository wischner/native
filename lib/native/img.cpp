...
namespace native
{
    ...
    img::img(dim w, dim h)
        ...
        _data =
            std::make_unique<rgba[]>(static_cast<std::size_t>(w) * h);
    }
    ...
    img img::decode(const std::uint8_t *data, std::size_t size) {
        if (!data || size == 0)
            throw std::invalid_argument(
                "img::decode: encoded data is empty");
        detail::decoded_image decoded =
            detail::decode_image(data, size);
        return img(
            decoded.width, decoded.height, std::move(decoded.pixels));
    }

    std::vector<std::uint8_t> img::encode(image_format format,
        ...
        return detail::encode_image(format,
                                    _data.get(),
                                    static_cast<dim>(_w),
                                    static_cast<dim>(_h),
                                    jpeg_quality);
