...
namespace
{
    ...
    void ensure_backbuffer(native::wnd *owner,
                           ...
                           int width,
                           int height) {
        if (!canvas || !linux::openmotif::cached_display ||
            width <= 0 || height <= 0)
            return;
    ...
    void handle_canvas_event(Widget widget,
        ...
        case Expose: {
            ...
            if (width <= 0 || height <= 0) {
                ...
                Dimension h = 0;
                XtVaGetValues(
                    widget, XmNwidth, &w, XmNheight, &h, nullptr);
...
namespace native
{
    ...
    void app_wnd::create() const {
        ...
        if (!linux::openmotif::app_instance) {
            ...
            shell = XtVaAppInitialize(&linux::openmotif::app_instance,
                                      ...
                                      nullptr);
            if (!shell)
                throw std::runtime_error("Motif: Failed to initialize "
        ...
        // can be attached via XmNmenuBar without violating the
    ...
    void app_wnd::show() const {
        ...
        if (!canvas)
            throw std::runtime_error(
                "Motif: Missing drawing widget binding for app_wnd.");

        XtRealizeWidget(shell);
        linux::openmotif::cached_display = XtDisplay(shell);

        // Prevent XClearArea from blanking the drawing surface before
        // Expose. The backbuffer XCopyArea should own what reaches the
        // screen.
        XSetWindowBackgroundPixmap(
            linux::openmotif::cached_display, XtWindow(canvas), None);

        linux::openmotif::wm_delete_window_atom =
            XmInternAtom(linux::openmotif::cached_display,
    ...
    void app_wnd::destroy() const {
        ...
        if (get_modal() && owner) {
            ...
            Widget focus_shell =
                focus ? linux::openmotif::shell_bindings
