//
// Implements the OpenMotif image-graphics backend.
//
...
namespace native
{
    ...
    gpx &gpx_img::draw_rect(rect r, bool filled) {
        ...
        const int x2 = std::min(
            static_cast<int>(_img.w() - 1),
            std::min(
                static_cast<int>(r.x2()),
                static_cast<int>(_clip.x2())));
        const int y2 = std::min(
            static_cast<int>(_img.h() - 1),
            std::min(
                static_cast<int>(r.y2()),
                static_cast<int>(_clip.y2())));
    ...
    gpx &gpx_img::draw_text(const std::string &text, point p) {
        ...
        XDrawString(
            display,
            reinterpret_cast<Drawable>(ximg->data),
            gc,
            p.x,
            p.y,
            text.c_str(),
