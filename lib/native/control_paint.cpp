//
// Implements native and portable control-part rendering.
//
...
namespace native
{
    ...
    control_paint &control_paint::draw_button_face(
        const rect &r,
        const state &s) {
        if (detail::control_paint_backend_draw_button_face_native(
                _g, r, s)) {
            return *this;
        }

        const palette p = native_palette();
        const rgba old_ink = _g.get_ink();
        const uint8_t old_pen = _g.get_pen();

        const rgba bg = s.pressed
                            ? p.button_pressed_bg
    ...
    control_paint &control_paint::draw_button_frame(
        const rect &r,
        const state &s) {
        if (detail::control_paint_backend_draw_button_frame_native(
                _g, r, s)) {
            return *this;
        }
    ...
    control_paint &control_paint::draw_button_text(
        const rect &r,
        const std::string &text,
        const state &s) {
        if (detail::control_paint_backend_draw_button_text_native(
                _g, r, text, s)) {
            return *this;
        }
        ...
        const rgba old_ink = _g.get_ink();
        const uint8_t old_pen = _g.get_pen();
        const font_t &old_font = _g.get_font();

        const rgba fg = s.pressed
                            ? p.button_pressed_text
                            : (s.hot
                                   ? p.button_hot_text
                                   : p.button_text);
        const int tw = detail::control_paint_backend_text_width(text);
        const int tx = r.p.x + std::max(
            0, (static_cast<int>(r.d.w) - tw) / 2);
        const int ty =
            detail::control_paint_backend_text_y_centered(r) +
            (s.pressed ? 1 : 0);

        _g.set_font(font_t::stock(font_role::control));
        _g.set_ink(fg).draw_text(
            text, point(tx + (s.pressed ? 1 : 0), ty));

        _g.set_font(old_font).set_pen(old_pen).set_ink(old_ink);
        return *this;
    }

    control_paint &control_paint::draw_button(
        const rect &r,
        const std::string &text) {
    ...
    control_paint &control_paint::draw_menu_bar_background(
        const rect &r) {
        if (detail::control_paint_backend_draw_menu_bar_background_native(
                _g, r)) {
            return *this;
        }
    ...
    control_paint &control_paint::draw_menu_item_background(
        const rect &r,
        const state &s) {
        if (detail::control_paint_backend_draw_menu_item_background_native(
                _g, r, s)) {
            return *this;
        }
        ...
        _g.set_pen(1);
        _g.set_ink(active ? p.menu_hot_bg : p.menu_popup_bg)
            .draw_rect(r, true);
    ...
    control_paint &control_paint::draw_menu_item_text(
        const rect &r,
        const std::string &text,
        const state &s) {
        if (detail::control_paint_backend_draw_menu_item_text_native(
                _g, r, text, s)) {
            return *this;
        }
    ...
    control_paint &control_paint::draw_menu_bar(const rect &r) {
        ...
        _g.set_ink(p.menu_bar_line_top)
            .draw_line(point(r.x1(), r.y1()), point(r.x2(), r.y1()));
        _g.set_ink(p.menu_bar_line_bottom)
            .draw_line(point(r.x1(), r.y2()), point(r.x2(), r.y2()));
        _g.set_pen(old_pen).set_ink(old_ink);
        return *this;
    }

    control_paint &control_paint::draw_menu_title(
        const rect &r,
        const std::string &text) {
    ...
    control_paint &control_paint::draw_menu_item(
        const rect &r,
        const std::string &text) {
    ...
    control_paint &control_paint::draw_list_item(
        const rect &r,
        const std::string &text) {
