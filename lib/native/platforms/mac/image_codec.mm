...
namespace native::detail
{
    decoded_image decode_image(const std::uint8_t *data,
        ...
        CFDataRef encoded = CFDataCreate(
            kCFAllocatorDefault, data, static_cast<CFIndex>(size));
        if (!encoded)
            throw std::bad_alloc();
        ...
        if (!source)
            throw std::runtime_error(
                "macOS image codec: decode failed");
        ...
        if (width == 0 || height == 0 ||
            ...
            throw std::runtime_error("macOS image codec: dimensions "
    ...
    std::vector<std::uint8_t> encode_image(image_format format,
        ...
        if (format == image_format::jpeg) {
            const double fraction =
                static_cast<double>(jpeg_quality) / 100.0;
            quality = CFNumberCreate(
                kCFAllocatorDefault, kCFNumberDoubleType, &fraction);
            const void *keys[] = {
        ...
        if (!finalized) {
            CFRelease(encoded);
            throw std::runtime_error(
                "macOS image codec: encode failed");
        }
